...
if [ ! -d ${source_files} ]
then
    echo "Problem with ${source_files}"
else
    cd ${HOME}
    ...
    if [ -f ${source_files}/.additional_startup ] && [ ! -f .additional_startup ]
    then
    ...
    then
        ...
        then
            ...
            ln -s ${source_files}/ssh-agent.service ${HOME}/.config/systemd/user/ssh-agent.service
        fi
    fi
fi
